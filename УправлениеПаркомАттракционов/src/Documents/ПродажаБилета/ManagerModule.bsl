...
// Обработчик перехода н ановую версию
...
Процедура ПеренестиНоменклатуруВТабличнуюЧасть() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		НоваяСтрока = ДокОбъект.ПозицииПродажи.Добавить();
		НоваяСтрока.Номенклатура = ДокОбъект.УдалитьНоменклатура;
		НоваяСтрока.Цена = ДокОбъект.УдалитьЦена;
