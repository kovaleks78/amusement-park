	...
	Для Сч = 0 По Диапазоны.Количество() - 1 Цикл
		...
		Если Сч > 0 Тогда
			Если Диапазон.НижняяГраница <> ВерхняяГраницаПредыдущегоДиапазона Тогда 
				Отказ = Истина;
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = СтрШаблон("Верхняя граница каждого диапазона должна быть равна нижней следующего.");
				Сообщение.УстановитьДанные(ЭтотОбъект);
		...
		Если Диапазон.НижняяГраница >= Диапазон.ВерхняяГраница 
			...
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтрШаблон("Верхняя граница всегда больше нижней границы.");
			Сообщение.УстановитьДанные(ЭтотОбъект);
